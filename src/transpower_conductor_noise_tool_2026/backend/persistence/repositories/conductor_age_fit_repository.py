...
class ConductorAgeFitRepository:
    def list_fits(self, noise_site_id=None, detection_logic=None, metric=None):
        query = ConductorAgeFit.query
        if noise_site_id:
            query = query.filter(ConductorAgeFit.noise_site_id.in_(noise_site_id))
        if detection_logic is not None:
            query = query.filter(ConductorAgeFit.detection_logic == detection_logic)
        if metric is not None:
            query = query.filter(ConductorAgeFit.metric == metric)
        return query.order_by(ConductorAgeFit.noise_site_id.asc()).all()
    ...
    def replace_all(self, records):
        ...
        # data to fit.
        table = ConductorAgeFit.__table__
