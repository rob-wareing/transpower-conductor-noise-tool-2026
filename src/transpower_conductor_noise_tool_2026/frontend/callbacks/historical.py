...
def register_callbacks(dash_app, backend_url: str | None):
    ...
    @dash_app.callback(
        ...
        Input("historical-export-button", "n_clicks"),
        State("historical-table", "data"),
        prevent_initial_call=True,
    )
    def export_historical(n_clicks, rows):
        if not n_clicks or not rows:
            return no_update
        return dcc.send_data_frame(pd.DataFrame(rows).to_csv, "historical.csv", index=False)
